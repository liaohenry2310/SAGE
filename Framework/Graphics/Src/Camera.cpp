...
void Sage::Graphics::Camera::LookAt(const Math::Vector3& target)
{
...
void Sage::Graphics::Camera::SetSize(float width, float height)
{
	mWidth = width;
	mHeight = height;
}
...
Math::Matrix4 Sage::Graphics::Camera::GetPerspectiveMatrix() const
{
...
Math::Matrix4 Sage::Graphics::Camera::GetOrthographicMatrix() const
{
	const float w = (mWidth == 0) ? GraphicsSystem::Get()->GetBackBufferWidth() : mWidth;
	const float h = (mHeight == 0) ? GraphicsSystem::Get()->GetBackBufferHeight() : mHeight;
